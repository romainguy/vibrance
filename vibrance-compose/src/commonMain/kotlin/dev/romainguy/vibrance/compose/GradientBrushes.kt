...
/**
 * Create a gradient shader from the specified start position ([startOffset]), to the
 * specified end position ([endOffset]). Use [Float.POSITIVE_INFINITY] for x and y to
 * indicate the far right and far bottom of the drawing area respectively.
 *
 * @param startOffset The starting point of the gradient.
 * @param startColor The color to interpolate from.
 * @param endOffset The ending point of the gradient.
...
internal class LinearPigmentsGradientBrush(
    ...
    override val intrinsicSize: Size
        get() =
            Size(
                if (startOffset.x.isFinite() && endOffset.x.isFinite()) {
                    abs(startOffset.x - endOffset.x)
                } else {
                    Float.NaN
                },
                if (startOffset.y.isFinite() && endOffset.y.isFinite()) {
