...
class Pigments():
    ...
    def mix_as_curve(self, concentration):
        ...
        return np.clip(np.array([
             -0.0002486 - 1.0875751 * c0 + 0.4042848 * c1 + 0.2906841 * c2
            + 0.3926048 * c3 + 1.5617771 * c0**2 - 0.6377270 * c0 * c1 - 1.0360933 * c0 * c2
            - 0.9755315 * c0 * c3 + 0.1185588 * c1**2 + 0.4534512 * c1 * c2 + 0.4700028 * c1 * c3
            + 0.1760716 * c2**2 + 0.6972558 * c2 * c3 + 0.2008756 * c3**2,
              0.1979408 - 0.0535665 * c0 - 0.3313803 * c1 + 0.1409142 * c2
            + 0.2440298 * c3 + 0.1063269 * c0**2 + 0.2765756 * c0 * c1 - 0.2544095 * c0 * c2
            - 0.1820590 * c0 * c3 + 0.2890032 * c1**2 - 0.4497897 * c1 * c2 - 0.4471765 * c1 * c3
            + 0.2997280 * c2**2 + 0.5453901 * c2 * c3 + 0.3278717 * c3**2,
            0.2514996 + 0.2738970 * c0 + 0.2436320 * c1 - 0.8279018 * c2
            + 0.3103758 * c3 + 0.1464629 * c0**2 + 0.1241566 * c0 * c1 - 0.5088547 * c0 * c2
            + 0.5121328 * c0 * c3 + 0.0118029 * c1**2 - 0.2984050 * c1 * c2 + 0.4060776 * c1 * c3
            + 0.8111606 * c2**2 - 0.8318062 * c2 * c3 + 0.2239711 * c3**2,
        ]), 0.0, 1.0)

    def mix_as_nn(self, concentration):
        c0, c1, c2, c3 = concentration
        r = -0.0249582
        g =  0.3584879
        b =  0.0244421

        h1 = max(0.0, 0.0098267 + c0 * 0.1174974 + c1 * 0.5078091 - c2 * 0.2443916 - c3 * 0.3047154)
        r -= h1 * 0.0341629
        g += h1 * 0.5760666
        b -= h1 * 0.0325691

        h2 = max(0.0, -0.6286458 - c0 * 2.8839049 + c1 * 0.7584528 + c2 * 0.7916569 + c3 * 0.7818422)
        r += h2 * 2.7300307
        g += h2 * 0.1375640
        b -= h2 * 0.0742932

        h3 = max(0.0, 0.1541230 + c0 * 0.2867913 + c1 * 0.0568774 - c2 * 0.5446743 + c3 * 0.1930918)
        r -= h3 * 0.1012921
        g += h3 * 0.0580865
        b += h3 * 0.5416284

        h5 = max(0.0, 0.0747424 - c0 * 1.7088713 + c1 * 0.1860320 + c2 * 0.2180080 + c3 * 0.2195648)
        r += h5 * 1.1088474
        g += h5 * 0.0263522
        b -= h5 * 0.0335531

        h6 = max(0.0, -0.2055153 - c0 * 0.4361817 - c1 * 0.8967318 + c2 * 0.4045247 + c3 * 0.4025162)
        r += h6 * 0.1815542
        g += h6 * 0.9910546
        b -= h6 * 0.1118827

        h9 = max(0.0, 0.3792505 + c0 * 0.0286469 + c1 * 0.4794231 - c2 * 0.3951822 + c3 * 0.0329820)
        r += h9 * 0.1625491
        g -= h9 * 0.6810130
        b += h9 * 0.2960019

        h10 = max(0.0, -0.1452454 + c0 * 0.2560401 + c1 * 0.1701091 - c2 * 1.2982900 + c3 * 0.3221915)
        r -= h10 * 0.1341101
        g += h10 * 0.0278039
        b += h10 * 1.2155828

        h11 = max(0.0, 0.2256926 - c0 * 0.1793450 - c1 * 0.3194858 - c2 * 0.0576552 + c3 * 0.5597214)
        r += h11 * 0.0951412
        g += h11 * 0.6306976
        b += h11 * 0.1533901

        h12 = max(0.0, 0.0271525 + c0 * 0.2065663 + c1 * 0.1657660 - c2 * 0.9326340 + c3 * 0.2456872)
        r += h12 * 0.0214799
        g += h12 * 0.0282143
        b += h12 * 0.8276884
